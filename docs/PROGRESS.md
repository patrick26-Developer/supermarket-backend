...
**`POST /api/uploads/product-image`** (`src/uploads/`, nouveau module) — `multipart/form-data`, champ `file`, `FileInterceptor` (`@nestjs/platform-express`, `multer` déjà présent en dépendance transitive — `diskStorage` vers `uploads/products/`, nom de fichier randomisé `randomUUID() + extension`), limite 5 Mo, filtre MIME (JPEG/PNG/WEBP/GIF uniquement), gardé derrière `PRODUCTS`/`UPDATE` (même permission que la modification d'un produit). Fichiers servis statiquement par `main.ts` (`app.useStaticAssets(join(process.cwd(), "uploads"), { prefix: "/uploads" })`, nécessite `NestFactory.create<NestExpressApplication>(...)`), hors du préfixe `/api`. Dossier `uploads/` ignoré par git (`.gitkeep` committé pour garder la structure).

**Piège : l'endpoint doit renvoyer une URL absolue, pas un chemin relatif** — première version retournait `{ url: "/uploads/products/xxx.png" }`. `CreateProductDto`/`UpdateProductDto` valident `imageUrl` avec `@IsUrl()`, qui **rejette un chemin sans protocole/hôte** → `PUT /products/:id` avec cette valeur échouait en 400, silencieusement pour l'utilisateur (juste "action impossible" côté UI, l'upload lui-même avait pourtant réussi). Corrigé en construisant l'URL absolue côté serveur à partir de la requête entrante (`${req.protocol}://${req.get("host")}/uploads/products/${file.filename}`) plutôt que de relâcher la validation — garde `imageUrl` toujours vérifiablement une vraie URL, uploadée ou externe.

**Piège esbuild/tsx : import de type sans `import type` fait planter le serveur au démarrage, silencieusement invisible à `tsc`** — `main.ts` importait `NestExpressApplication` (un type pur, aucune valeur d'exécution) via un `import` classique. `tsc --noEmit` ne l'a **pas** signalé malgré `verbatimModuleSyntax: true` dans `tsconfig.json` (à investiguer si ça se reproduit — inattendu). Mais `tsx` (esbuild, transformation fichier-par-fichier sans analyse de types) ne peut pas savoir que ce nom ne désigne qu'un type, laisse l'import tel quel dans le JS émis, et Node plante immédiatement : `SyntaxError: The requested module '@nestjs/platform-express' does not provide an export named 'NestExpressApplication'`. Corrigé en `import type { NestExpressApplication } from "@nestjs/platform-express"`. **Retenir : sur ce projet (esbuild-only, sans `emitDecoratorMetadata` fiable ni vérif de type à l'exécution), tout import qui n'est utilisé qu'en position de type doit être explicitement `import type`, même quand `tsc --noEmit` ne le réclame pas** — s'ajoute aux pièges esbuild déjà documentés (`@Inject()` explicite, `ValidateBodyPipe`, `Temporal.Instant`).

**Vérifié de bout en bout, pas juste par le typecheck** — upload d'un vrai fichier via le client Electron (Playwright, formulaire glisser-déposer), URL absolue reçue, produit enregistré, puis **revérifié indépendamment par requête API directe** (`GET /products?search=...` après l'upload) : `imageUrl` bien persisté en base avec la nouvelle URL, fichier re-téléchargeable (`GET /uploads/products/<id>.png` → 200, `image/png`).

**Note d'environnement, pas un bug applicatif** : cette session a tourné sur une machine ponctuellement chargée à 100 % CPU (confirmé via `typeperf`, processus concurrents WSL/Docker Desktop/Kubernetes local/WPS) — une connexion (`POST /auth/login`, bcrypt + signature JWT) a pris jusqu'à 20 s au lieu de la centaine de ms habituelle, un démarrage `tsx watch` a pris plus de 15 minutes au lieu de ~1 minute. Confirmé par mesure directe (`curl` chronométré, hors Electron/Playwright) que le serveur répondait correctement, juste lentement — aucune correction de code nécessaire pour ce point, seulement des délais d'attente de test rallongés.

## 2026-09-02 — Démarrage du client Electron (`supermarket-desktop`)

Phase 2 du backend confirmée stable ("prêt, stable et évolutif"), démarrage du client desktop dans `Projets/app-desktop/superette/supermarket-desktop/` (dossier frère de `supermarket-backend/`), stack demandée : Electron Forge + React 19 + Vite + TypeScript + TailwindCSS v4 + shadcn/ui (style "new-york") + lucide-react.

**Scaffold** : `npm init electron-app@latest supermarket-desktop -- --template=vite-typescript` (template vanilla, pas de React par défaut — ajouté manuellement par-dessus). React monté dans `#root` via `createRoot` (`src/renderer.tsx`), `tsconfig.json` réécrit (`jsx: react-jsx`, `moduleResolution: bundler`, alias `@/*`), Tailwind v4 câblé via `@tailwindcss/vite` dans `vite.renderer.config.ts`, shadcn/ui posé **à la main** (`components.json`, `src/lib/utils.ts`, `Button`/`Input`/`Label`/`Card`) — le CLI `npx shadcn@latest init` reste bloqué indéfiniment en environnement non interactif (aucune sortie, aucun prompt visible), abandonné au profit d'une pose manuelle conforme au style "new-york" généré habituellement par le CLI.

**Écran de connexion réel, câblé sur le backend** — `src/lib/api.ts` (client fetch minimal, `VITE_API_URL` configurable, défaut `http://localhost:3000/api`) + `src/pages/LoginPage.tsx` (email/mot de passe, shadcn `Card`/`Input`/`Button`) + `src/App.tsx` (bascule login ↔ écran post-connexion selon l'état React). Vérifié de bout en bout, fenêtre Electron réelle pilotée par Playwright (`_electron`), contre le vrai backend Docker : connexion avec `admin@superette.local` → JWT reçu → écran post-login affichant `Admin Système · SUPER_ADMIN` (voir captures dans le rapport de session).

**Quatre pièges rencontrés et corrigés, tous spécifiques à Electron Forge + Vite + Tailwind v4 :**

1. **`@tailwindcss/vite` est ESM-only**, mais le plugin Vite d'Electron Forge charge `vite.renderer.config.ts` en CommonJS (le `package.json` du projet n'a pas `"type": "module"`) → `require()` d'un module ESM échoue à la construction (`npm run package`). Passer `"type": "module"` au `package.json` **casse à son tour** le process main (`main.js` généré par Forge en CJS, avec du `require()` interne — `ReferenceError: require is not defined in ES module scope`). **Solution retenue** : garder le `package.json` en CommonJS et importer le plugin Tailwind dynamiquement dans une factory async (`defineConfig(async () => { const { default: tailwindcss } = await import("@tailwindcss/vite"); ... })`) — fonctionne aussi bien en CJS qu'en ESM, aucune autre config à toucher.
2. **`tsconfig.json` avec un `include: ["src"]` trop strict** faisait perdre `forge.env.d.ts` (déclare les globales injectées par le plugin Vite, `MAIN_WINDOW_VITE_DEV_SERVER_URL`/`MAIN_WINDOW_VITE_NAME`) → 3 erreurs `tsc --noEmit`. Corrigé en incluant explicitement `forge.env.d.ts` et les fichiers de config `.ts` à la racine.
3. **`electron` (devDependency) n'avait pas son binaire extrait** (`node_modules/electron/dist/` absent malgré un `npm install` propre — le zip était bien en cache local mais `install.js` ne l'avait pas extrait). `npx electron-forge package`/`start` fonctionnaient quand même (le packager sait re-déclencher le téléchargement), mais piloter le binaire directement (`node_modules/electron/dist/electron.exe`, nécessaire pour l'automatisation Playwright) échouait silencieusement. Corrigé par `node node_modules/electron/install.js` manuel.
4. **Fuse `EnableNodeCliInspectArguments: false`** (`forge.config.ts`, activé par défaut par le template pour la sécurité) empêche Playwright de piloter un **build packagé** (`out/*.exe`) — il ne peut jamais attacher le protocole d'inspection Node, `_electron.launch()` reste bloqué en timeout. Contournement pour les vérifications automatisées : piloter le build **non packagé** (`.vite/build/` généré par `npm run package` avant l'étape de packaging, lancé directement via `node_modules/electron/dist/electron.exe <dossier-projet>`) plutôt que l'exécutable final — le fuse ne s'applique qu'à l'exécutable packagé, pas au binaire Electron brut.

**Piège d'environnement (spécifique à cette machine, pas au projet)** : la variable `ELECTRON_RUN_AS_NODE=1` est positionnée globalement dans cet environnement Windows — tout `electron.exe` lancé en héritant cet environnement s'exécute comme un Node.js ordinaire (`require('electron')` renvoie une chaîne, pas l'API `{ app, BrowserWindow }`), d'où un plantage silencieux (`Cannot read properties of undefined (reading 'on')`) qui n'a rien à voir avec le code de l'app. Pour tout lancement piloté (Playwright ou autre), retirer explicitement cette variable de l'environnement transmis au process.

**Corrections de qualité en cours de route** : fenêtre par défaut passée de 800×600 à 1280×800 (`minWidth`/`minHeight` 1024×640, plus réaliste pour un poste de caisse) ; `openDevTools()` déplacé sous la branche dev uniquement (le template l'appelait sans condition, donc aussi dans un build "production" packagé) ; `.eslintrc.json` du template ne déclarait pas `parserOptions.ecmaFeatures.jsx` — `npm run lint` aurait échoué sur tout fichier `.tsx`.

**Pas encore fait** : aucun écran métier au-delà du login (caisse, catalogue, stock…), pas de stockage sécurisé du token (`safeStorage`), pas de refresh automatique, pas de `docs/` propre au frontend (à créer). Pas de script/skill de pilotage Electron committé dans le projet — la vérification Playwright de cette session était un script jetable dans le scratchpad, à refaire au besoin (`/run-skill-generator` recommandé si ce pilotage doit devenir récurrent).
